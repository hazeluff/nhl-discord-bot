...
public class GameDayChannelsManagerTest {
	...
	@Before
	public void before() {
		gameDayChannelsManager = new GameDayChannelsManager(mockCanucksBot);
	...
	public void runShouldInvokeMethodsAndSleep() {
		...
		when(mockCanucksBot.getGameScheduler().getLastUpdate()).thenReturn(null, null, today, today, tomorrow, tomorrow,
				tomorrow);
	...
	public void createChannelShouldFunctionCorrectly() {
		...
		doReturn(gameDayChannel).when(spyGameDayChannelsManager).getGameDayChannel(any(CanucksBot.class),
		...
		when(mockCanucksBot.getGameScheduler().getGameTracker(game)).thenReturn(null);
		assertNull(spyGameDayChannelsManager.createChannel(game, guild));
		verify(spyGameDayChannelsManager, never()).getGameDayChannel(any(CanucksBot.class), any(GameTracker.class),
				any(Guild.class));
		verify(spyGameDayChannelsManager, never()).addGameDayChannel(anyLong(), anyInt(), any(GameDayChannel.class));

		// GameDayChannel doesn't exist; game tracker exists
		doReturn(null).when(spyGameDayChannelsManager).getGameDayChannel(guildId, gamePk);
		when(mockCanucksBot.getGameScheduler().getGameTracker(game)).thenReturn(gameTracker);
		assertEquals(gameDayChannel, spyGameDayChannelsManager.createChannel(game, guild));
		verify(spyGameDayChannelsManager).getGameDayChannel(mockCanucksBot, gameTracker, guild);
