...
/**
 * Interface for commands that the CanucksBot can accept and the replies to those commands.
 */
public abstract class Command {
	static final Consumer<MessageCreateSpec> SUBSCRIBE_FIRST_MESSAGE = spec -> spec
			.setContent("Please have your admin first subscribe your guild "
					+ "to a team by using the command `@CanucksBot subscribe [team]`, "
	...
	Game getLatestGame(Team team) {
		Game game = canucksBot.getGameScheduler().getCurrentGame(team);
		if (game == null) {
			game = canucksBot.getGameScheduler().getLastGame(team);
		}
		return game;
	}
