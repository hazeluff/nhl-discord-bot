	...
	public void run() {
		...
		if (lastMessageId != null) {
			channel.getMessagesBefore(lastMessageId).collectList().block().stream()
					...
					.forEach(message -> canucksBot.getDiscordManager().deleteMessage(message));
			canucksBot.getDiscordManager().deleteMessage(
					canucksBot.getDiscordManager().block(channel.getLastMessage()));
		}
		canucksBot.getDiscordManager().sendMessage(channel, UPDATED_MESSAGE);
		canucksBot.getDiscordManager().sendMessage(channel, aboutCommand.getReply());
		canucksBot.getDiscordManager().sendMessage(channel, helpCommand.getReply());
	}
}
